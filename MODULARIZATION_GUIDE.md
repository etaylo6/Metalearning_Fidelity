...
### 1. GAM with Main Effects Only
**File**: `gam_main_effects.py`

**What it does**: Uses a Generalized Additive Model (GAM) with main effects only (no interactions) to predict accuracy. This is a simpler, more interpretable version of GAM that models each feature independently.

**Key Features**:
- Interpretable component functions (you can see how each parameter affects accuracy independently)
- Main effects only: No interactions between parameters
...
- Faster training than GAM² (no interaction terms)

**Architecture**:
- Main effects: Smooth splines for continuous parameters, linear terms for binary features
- No interactions: Each feature contributes independently
- Output: Bounded [0, 1] predictions via clipping

**Advantages**:
- Highly interpretable (each parameter's effect is independent)
- Faster training than GAM²
- Less prone to overfitting with fewer parameters
- Good for understanding individual parameter effects

**Disadvantages**:
- Cannot capture interactions between parameters
- May be less accurate if parameter interactions are important
- `pygam` can be slower than PyTorch-based NNs

**Dependencies**: `pygam`, `numpy`, `matplotlib`, `pandas`

---

### 2. GAM² with Tensor Product Interactions
...
|-------|------|------------------|------------|----------------|-------------------|------------------|-------------|
| GAM | Statistical | ⭐⭐⭐⭐⭐ | Low | Fast | Medium | Partial | Yes |
...
- `pygam`: Generalized Additive Models (used by GAM and GAM²)
...
3. **View results**:
   ...
   - For GAM and GAM²: CSV logs saved in `logs/` directory with timestamp
...
- **Logs** (GAM and GAM²): CSV files with training/testing data and predictions
...
1. **Start simple**: Begin with GAM or GAM² for interpretability, or Simple NN for a baseline
...
- Examine example outputs in `logs/` directory (for GAM and GAM²)
...
- **GAM**: Highly interpretable statistical model with main effects only
